...
fun Meal.extractValidIngredients(): List<Pair<String, String>> {
    ...
    return ingredients.filter { (ingredient, _) ->
        !ingredient.isNullOrBlank()
    }.map { (ingredient, measure) ->
        Pair(ingredient.orEmpty(), measure.orEmpty())
    }
}